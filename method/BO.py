...
def BO(
    ...
    Init_now = Init
    if Gym_mode == 1:
        if not os.path.exists('{}/gym/{}d/{}/{}'.format(Exper_floder, Xdim, Seed, Task.name)):
            os.makedirs('{}/gym/{}d/{}/{}'.format(Exper_floder, Xdim, Seed, Task.name))
    else:
    ...
    if Method.split('_')[0] != 'BO' and Gym_mode == 0:
        for y in train_y:
            KB.random_y.append(y)

    if Gym_mode == 1:
        initial_x = np.copy(train_x)
        initial_y = np.copy(train_y)
    else:
        initial_x = None
        initial_y = None
    ...
    for i in range(Init_now, Evol):
        ...
        if Gym_mode == 1:
            mean = np.mean(initial_y)
            std =np.std(initial_y)
            train_y_norm = Normalize_mean_std(train_y, mean, std)
        elif Method.split('_')[0] != 'BO':
        ...
        #         Plot.plot_contour('{}_bo'.format(i), Task.name, model, Task, acquisition,
        #                           train_x, train_y, suggested_sample, acq_value,
        #                           'BO_GYM', KB, MOGP=False, Seed=Seed, test_size=101,
        #                           show=False, dtype=Dty, Exper_floder=Exper_floder)
        #     elif Xdim == 1:
        #         Plot.plot_one_dimension('{}_bo'.format(i), Task.name, model, Task,
        #                                 acquisition, train_x, train_y, suggested_sample, acq_value,
        #                                 'BO_GYM', KB, MOGP=False, Seed=Seed, show=False,
        #                                 dtype=Dty, Exper_floder=Exper_floder)
    ...
    if Gym_mode == 1:
        KB.add(Task.name, 'Gym', train_x, train_y, grid_x, initial_x=initial_x, initial_y=initial_y)
        np.savetxt('{}/gym/{}d/{}/{}/Gym_train_x.txt'.format(Exper_floder, Xdim, Seed, Task.name), train_x)
        np.savetxt('{}/gym/{}d/{}/{}/Gym_train_y.txt'.format(Exper_floder, Xdim, Seed, Task.name), train_y)
        with open('{}/gym/{}d/{}_GKB.txt'.format(Exper_floder, Xdim, Seed), 'wb') as f:  # 打开文件
            pickle.dump(KB, f)
    else:
