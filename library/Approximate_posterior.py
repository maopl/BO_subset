...
class Approximate_Posterior(object):
    """
    ...
    def __init__(self, woodbury_chol=None, woodbury_vector=None, K=None, mean=None, cov=None, K_chol=None,
                 woodbury_inv=None, sigma = None, prior_mean=0):
        """
    ...
    # def covariance_between_points(self, kern, pseudo_point, X1, X2):
    #     """
    #     Computes the posterior covariance between points.
    #
    #     :param kern: GP kernel
    #     :param pseudo_point: current input observations
    ...
    #
    #     Kx1 = kern.K(pseudo_point, X1)
    #     Kx2 = kern.K(pseudo_point, X2)
    #     K12 = kern.K(X1, X2)
    #
    #
    #     var = K12 - K
    ...
    def _raw_predict(self, kern, Xnew, pseudo_point, full_cov=False):
        woodbury_vector = self.woodbury_vector
        woodbury_inv = self.woodbury_inv

        if not isinstance(Xnew, VariationalPosterior):
            Kx = kern.K(pseudo_point, Xnew)
            ...
            psi0_star = kern.psi0(pseudo_point, Xnew)
            psi1_star = kern.psi1(pseudo_point, Xnew)
            psi2_star = kern.psi2n(pseudo_point, Xnew)
            la = woodbury_vector
...
class PosteriorApproximate_inducing(Approximate_Posterior):
    def _raw_predict(self, kern, Xnew, pred_var, full_cov=False):

        K_star = kern.K(pred_var, Xnew)
        mu = np.dot(K_star.T, self.woodbury_vector)
        if len(mu.shape) == 1:
            mu = mu.reshape(-1, 1)

        K_stst = kern.Kdiag(Xnew)

        tmp = K_star.T @ self.woodbury_inv @ K_star
        # tmp = dtrtrs(self._woodbury_chol, K_star)[0]
        var = (K_stst - np.square(tmp).sum(0))[:, None] + self._sigma

        # var = var
        return mu, var


class PosteriorApproximate_RSVD(Approximate_Posterior):
    def _raw_predict(self, kern, Xnew, pred_var, full_cov=False):
        Kx = kern.K(pred_var, Xnew)
        mu = np.dot(Kx.T, self.woodbury_vector)
        if len(mu.shape) == 1:
        ...
        tmp = Kx.T @ self.woodbury_inv @ Kx
        var = (Kxx - np.diag(tmp))[:, None]
